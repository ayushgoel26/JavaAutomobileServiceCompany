...
public class EmployeeResource {
	...
	public String addEmployee(@FormParam("empId") String empId, @FormParam("empPassword") String empPassword,
			@FormParam("empName") String empName, @Context HttpServletRequest request,
			@Context HttpServletResponse response) {

		String status = "Not Added";
		int result = 0;
		try {
			result = empDao.add(new Employee(empName, Long.parseLong(empId), empPassword));
	...
	public Response updateDetails(@FormParam("empId") long empId, @FormParam("property") String property,
		...
		try {
			result = Response.status(200).entity(empDao.update(empId, property, newValue)).build();
		} catch (SQLException e) {
			e.printStackTrace();
		}
		return result;
